...
df = pd.read_json("resultsMSVC_Mem.json", orient='split');
dc = pd.read_json("resultsGCC_Mem.json", orient='split');
# print(dc);

fig = make_subplots(rows=1, cols=2, x_title="Number of Entries", y_title="Memory Used (bytes)", subplot_titles=["MSVC", "GCC"], figure = go.Figure(layout= {
    "title": "Standard Vector vs Typeless Vector vs TypesafeTypeless Vector (Memory Usage)",
    "font": dict(
        family="Roboto Mono",
        size=18,
        color="Black"
...
fig.write_html('results_Mem.html', auto_open=True)
